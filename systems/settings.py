...
class SettingsSystem:
    ...
    def create_settings_view(self, refresh_callback=None) -> ft.Column:
        """创建设置视图"""
        ...
        return ft.Column(
            controls=[
                ...
                ft.Container(
                    content=ft.Column(
                        controls=[
                            ft.Text("【励志库】", size=14, color=ThemeConfig.TEXT_SECONDARY),
                            self._create_lizhi_settings(),
                        ],
                        spacing=10,
                    ),
...
• 定期维护重要关系
    ...
    def _create_lizhi_settings(self):
        """创建励志库管理界面"""
                    ...
                    content=ft.Row(
                        controls=[
                            ...
                            ft.IconButton(
                                ...
                                tooltip="删除",
                                data=quote_id,
                                on_click=lambda e, qid=quote_id, qcontent=content: self._delete_quote(e, qid, qcontent),
                            ),
                        ],
                        spacing=10,
                    ),
        ...
        if not quote_list:
            quote_list.append(
                ft.Text(
                    "还没有添加任何励志语录",
                    size=12,
                    color=ThemeConfig.TEXT_DISABLED,
                    italic=True,
                )
            )
                ...
                ft.Row(
                    controls=[
                        ...
                        ft.OutlinedButton(
                            f"查看全部 ({len(quotes)})",
                            icon=ft.icons.LIST,
                            on_click=self._show_all_quotes,
                        ),
                    ],
                    spacing=10,
                ),
    ...
    def _add_quote_dialog(self, e):
        ...
        def save_quote(e):
            ...
            if not content:
                content_input.error_text = "请输入内容"
                page.update()
                return

            # 保存（不需要作者，使用空字符串）
            success = self.db.add_quote(content, "", "poetry")
            ...
            if success:
                ...
                content_input.error_text = "添加失败，请重试"
                page.update()
    ...
    def _delete_quote(self, e, quote_id: int, content: str):
        """删除励志语录"""
        page = e.page
        ...
        def confirm_delete(e):
            ...
            if success:
                ...
                dialog.open = False
                page.update()
                self._show_message(page, "错误", "删除失败，请重试")
    ...
    def _show_all_quotes(self, e):
                    ...
                    content=ft.Row(
                        controls=[
                            ...
                            ft.IconButton(
                                ...
                                icon_color=ThemeConfig.DANGER_COLOR,
                                tooltip="删除",
                                on_click=lambda e, qid=quote_id, qcontent=content: self._delete_quote_from_all(e, qid, qcontent, dialog),
                            ),
                        ],
                        spacing=10,
                    ),
        ...
        dialog = ft.AlertDialog(
            title=ft.Text(f"所有励志语录 ({len(quotes)})"),
            content=ft.Container(
                content=ft.Column(
                    controls=quote_widgets,
                    spacing=8,
                    scroll=ft.ScrollMode.AUTO,
                ),
                width=500,
    ...
    def _delete_quote_from_all(self, e, quote_id: int, content: str, parent_dialog):
        """从查看全部对话框中删除语录"""
        page = e.page
